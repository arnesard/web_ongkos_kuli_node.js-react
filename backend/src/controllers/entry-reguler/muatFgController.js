...
async function list(req, res) {
  ...
  let sql = `
    SELECT dt.*, dk.biaya_truk, (dt.qty_truk * dk.biaya_truk) AS total_biaya
    FROM ${TABLE} dt
  ...
  try {
    ...
    const [customerRows] = await pool.query("SELECT * FROM data_kota_tbl ORDER BY customer");
    ...
    return fail(res, "Gagal mengambil data muat FG.", 500);
  }
}

// POST /api/entry-reguler/muat-fg
async function create(req, res) {
  ...
  try {
    ...
    console.error("[muatFg.create]", err);
    return fail(res, "Gagal menyimpan data.", 500);
  }
}

// PUT /api/entry-reguler/muat-fg/:id
async function update(req, res) {
  ...
  try {
    ...
    console.error("[muatFg.update]", err);
    return fail(res, "Gagal memperbarui data.", 500);
  }
}

// DELETE /api/entry-reguler/muat-fg/:id
async function remove(req, res) {
  ...
  try {
    ...
    console.error("[muatFg.remove]", err);
