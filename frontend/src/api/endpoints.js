...
function makeCrudApi(basePath) {
  return {
    ...
    update: async (id, payload) =>
      (await api.put(`${basePath}/${id}`, payload)).data.data,
...
export const bonSementaraApi = {
  ...
  list: async (params) =>
    (await api.get("/entry-reguler/bon-sementara", { params })).data.data.data,
  inputAktual: async (payload) =>
    (await api.post("/entry-reguler/bon-sementara/input-aktual", payload)).data,
  ...
  cariNoDoc: async (no_doc) =>
    (
      await api.get("/entry-reguler/bon-sementara", {
        params: { cari: 1, no_doc },
      })
    ).data.data,
  // daftar no_doc yang sudah pernah diinput (buat dropdown/combobox), di-scope warehouse otomatis di backend
  noDocs: async () =>
    (await api.get("/entry-reguler/bon-sementara")).data.data.noDocs,
  ...
  recent: async ({ limit = 20, bulanKode } = {}) =>
    (
      await api.get("/entry-reguler/bon-sementara/recent", {
        params: { limit, bulanKode },
      })
    ).data.data.data,
};
...
export const muatFgApi = {
  ...
  list: async (params) =>
    (await api.get("/entry-reguler/muat-fg", { params })).data.data.datas,
  meta: async (params) =>
    (await api.get("/entry-reguler/muat-fg", { params })).data.data,
};

export const bongkarRmApi = {
  ...makeCrudApi("/entry-reguler/bongkar-rm"),
  list: async (params) =>
    (await api.get("/entry-reguler/bongkar-rm", { params })).data.data.datas,
  meta: async (params) =>
    (await api.get("/entry-reguler/bongkar-rm", { params })).data.data,
};
...
export const uangMakanApi = {
  ...
  list: async (params) =>
    (await api.get("/entry-nonreguler/uang-makan", { params })).data.data
      .dataUangMakan,
  meta: async (params) =>
    (await api.get("/entry-nonreguler/uang-makan", { params })).data.data,
};
...
export const susunTireApi = {
  ...
  list: async (params) =>
    (await api.get("/entry-nonreguler/susun-tire", { params })).data.data.datas,
  meta: async (params) =>
    (await api.get("/entry-nonreguler/susun-tire", { params })).data.data,
  getLastKode: async (params) =>
    (await api.get("/entry-nonreguler/susun-tire/last-kode", { params })).data
      .data,
};
...
export const pemindahanBarangApi = {
  ...
  list: async (params) =>
    (await api.get("/entry-nonreguler/pemindahan-barang", { params })).data.data
      .dataPemindahan,
  meta: async (params) =>
    (await api.get("/entry-nonreguler/pemindahan-barang", { params })).data
      .data,
};
...
export const managementApi = {
  ...
  approveList: async (params) =>
    (await api.get("/management/approve-bongkarmuat", { params })).data.data,
  ...
  cetakNotaKuli: async (params) =>
    (await api.get("/management/performance-kuli/cetak-nota", { params })).data
      .data,
  balanceCash: async (params) =>
    (await api.get("/management/balance-cash", { params })).data.data,
  balanceCashResume: async (tanggal, warehouse) =>
    (
      await api.get(`/management/balance-cash/resume/${tanggal}`, {
        params: { warehouse },
      })
    ).data.data,
  balanceCashDetail: async (tanggal, warehouse) =>
    (
      await api.get(`/management/balance-cash/detail/${tanggal}`, {
        params: { warehouse },
      })
    ).data.data,
  // no_doc_b64 sudah di-encode (base64 lalu encodeURIComponent) sebelum dikirim ke sini
  bsReport: async (noDocB64) =>
    (await api.get(`/management/transaksi-bs/${noDocB64}`)).data.data,
  lpbsReport: async (noDocB64) =>
    (await api.get(`/management/transaksi-lpbs/${noDocB64}`)).data.data,
};
...
export const lookupApi = {
  customer: async (params) =>
    (await api.get("/helpers/customer", { params })).data.data,
  kuliList: async () => (await api.get("/helpers/kuli-list")).data.data,
  getLastKode: async (params) =>
    (await api.get("/helpers/get-last-kode", { params })).data.data,
