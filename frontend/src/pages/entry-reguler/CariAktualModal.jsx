...
// Jenjang approval asli (samain sama backend/src/controllers/management/approveController.js):
// kosong -(SH)-> approvebysh -(DH)-> approvebydh -(HOD)-> approve
const STATUS_LABELS = {
  approvebysh: "SH Approved",
  approvebydh: "DH Approved",
  approve: "Approved",
};

const statusBadge = (s) => {
  const val = s || "";
  const cls = val.includes("reject")
    ? "danger"
    : val === "approve"
      ? "success"
      : val
 ...
 *   buat level 'superuser' & 'admin' (approved atau belum, backend jadi penjaga utama).
 */
export default function CariAktualModal({ open, onClose, onSaved }) {
  const { user } = useAuth();
  const userLevel = String(user?.level || "").toLowerCase();
  const canInputAktual = userLevel === "superuser" || userLevel === "admin";
  ...
  const loadRecent = async (kode) => {
    ...
    try {
      const rows = await bonSementaraApi.recent({
        limit: 20,
        bulanKode: kode || undefined,
      });
      setRecentList(rows);
    } catch (err) {
    ...
    try {
      ...
      if (!payload.dataCari || payload.dataCari.length === 0) {
        Swal.fire({
          ...
          text: `No dokumen "${docToSearch}" tidak ada di data kamu.`,
        });
        return;
      }
      setResult(payload);
      ...
      Swal.fire({
        ...
        text:
          ...
          "Terjadi kesalahan saat mencari dokumen.",
      });
    } finally {
      setSearching(false);
    }
  ...
  const handleSaveAktual = async () => {
    ...
    setSaving(true);
    try {
      await bonSementaraApi.inputAktual({
        no_doc: noDoc.trim(),
        act_nilai: actNilai,
      });
      Swal.fire({
      ...
      Swal.fire({
        ...
        text:
          err.response?.data?.message ||
          "Terjadi kesalahan saat menyimpan nilai aktual.",
      });
    } finally {
      setSaving(false);
    }
      ...
      {result && (
        <div className="form-neo" style={{ padding: "0 20px 20px" }}>
          ...
          >
            ...
            {isLocked && (
              <span
                className={`badge-neo ${String(result.statusBS).includes("reject") ? "danger" : "success"}`}
                style={{ display: "inline-flex", alignItems: "center", gap: 4 }}
              >
                <Lock size={12} />
                {String(result.statusBS).includes("reject")
                  ? "Ditolak"
                  : `Sudah ${STATUS_LABELS[result.statusBS] || result.statusBS}`}
              </span>
            )}
          </div>

          {/* Isi nilai aktual — role-based: SuperUser & Admin selalu bisa (approved
              atau belum), level lain sama sekali nggak bisa */}
          {canInputAktual ? (
            ...
            <p style={{ fontSize: 12, opacity: 0.7, margin: 0 }}>
              Hanya SuperUser/Admin yang dapat mengisi nilai aktual.
            </p>
          )}
        </div>
      )}
